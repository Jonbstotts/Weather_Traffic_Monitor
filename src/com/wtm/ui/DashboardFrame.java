...
public final class DashboardFrame extends JFrame {
    ...
    private ScheduledFuture<?> sportsTask;
    ...
    private void updateWidgets(){
        ...
        for(Component c:sidePanel.getComponents()){
            ...
            String type=(String)((JComponent)p).getClientProperty("widgetType");
            ...
            else if("WIND_PRIMARY".equals(type)){
                WeatherSnapshot w=weather.get(config.primary.name());
                body.setText(w==null?"<html>Loading…</html>":"<html><b>"+Math.round(w.windMph())+" mph</b> sustained<br>Gusts up to <b>"+Math.round(w.gustMph())+" mph</b></html>");
            }
    ...
    private String statusHtml(){
        ...
        String traffic=(config.tomTomApiKey==null||config.tomTomApiKey.isBlank())
                ?"Traffic: "+config.trafficProvider+" • credential not configured"
                :"Traffic: "+config.trafficProvider+" • credential configured";
        String weather="Weather: "+config.weatherProvider
                +("OPEN_METEO_CUSTOMER".equalsIgnoreCase(config.weatherProvider)
                    ? ((config.weatherApiKey==null||config.weatherApiKey.isBlank())?" • credential missing":" • credential configured")
                    : " • keyless");
    ...
    private synchronized void startRefreshers(){
        ...
        sportsTask=scheduler.scheduleAtFixedRate(
                this::refreshSports,
                0,
                Math.max(2,config.sportsRefreshMinutes),
                TimeUnit.MINUTES);
    ...
    private void applyTheme(){
         ...
         * themes use the dark basemap/traffic style; light-family themes use
         * the standard light basemap.
         */
        config.darkMode=active.dark();
        ...
        for(Component c:allComponents(root)){
            if(c instanceof RoundedPanel rp){
                ...
                if("forecast".equals(role)) rp.setBackground(Theme.panel2());
                else rp.setBackground(Theme.panel());
        ...
        if(tickerPanel!=null){
            tickerPanel.setBackground(Theme.panel());
    ...
    private static int parseSportsIndex(String type){
        try{return Integer.parseInt(type.substring("SPORTS_".length()));}
        catch(Exception e){return -1;}
    }
